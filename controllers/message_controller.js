...
exports.getMessages = async (req, res) => {
  ...
  const findFriend = await User.findOne({ email: friendEmail });
  if (!findFriend) {
    return res.status(404).json({ message: "Usuário do amigo não encontrado" });
  }

  var listMessages;
  for (const amigo of findUser.amigos) {
    if (amigo.email === findFriend.email) {
      listMessages = amigo.messages;
    }
  }
...
exports.fixedMessage = async (req, res) => {
  try {
    ...
    const fixedMessage = await Message.findByIdAndUpdate(
      id,
      { fixed: true },
      { new: true },
    );
